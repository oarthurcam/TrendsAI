    ...
    <div className="bg-card p-6 rounded-xl flex flex-col items-center justify-center text-center gap-4 shadow-sm">
      <div
        ...
        className={`w-full p-8 border-2 border-dashed rounded-xl flex flex-col items-center justify-center transition-colors duration-200
          ${isLoading ? 'cursor-not-allowed bg-slate-100' : 'cursor-pointer'}
          ${isDragging ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/70 hover:bg-slate-50'}`}
      ...
      <button
        ...
        className="w-full inline-flex items-center justify-center px-4 py-2.5 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-primary hover:bg-primary/90 disabled:bg-slate-200 disabled:cursor-not-allowed disabled:text-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-primary transition-all duration-300"
      >
        {isLoading ? (
          <>
