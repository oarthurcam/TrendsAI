...
const responseSchema = {
  ...
  properties: {
    ...
    keyInsights: {
      type: Type.STRING,
      description: "Os insights-chave da análise. O formato deve ser dinâmico e adaptado ao contexto dos dados: às vezes como uma lista de pontos (usando '*' para cada item), às vezes como um parágrafo narrativo (use quebras de linha '\\n' para melhor legibilidade), e outras vezes como perguntas reflexivas.",
    },
    suggestedChart: {
      type: Type.OBJECT,
...
export const analyzeExcelData = async (csvData: string): Promise<AnalysisResult> => {
  ...
  const prompt = `
    ...
    5.  **Insights-Chave (Formato Variável)**: Apresente os insights mais importantes. NÃO siga um padrão fixo. Varie a apresentação:
        - Às vezes, use uma lista de pontos (inicie cada item com '* ').
        - Outras vezes, escreva um parágrafo narrativo coeso. Para facilitar a leitura, separe os parágrafos com quebras de linha.
        - Ou formule de 2 a 3 perguntas reflexivas que os dados levantam.
        - Adapte a redação e o tom ao contexto dos dados.
